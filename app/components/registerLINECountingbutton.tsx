...
export const RegisterCountingLine: React.FC<props> = ({addCoutingLine, DeleteCoutingLine, data, id_head_Couting, id_head_sheet}) => {
    const [idfactura, setIdfactura] = useState<number>(0);
    const [addLine, setAddline] = useState<boolean>(false);
    const [CoutingLine, setCoutingLine] = useState<CoutingDatail>(createEmptyCoutingLine());
    ...
    const SaveLineDetail = () => {
        ...
        setCoutingLine(createEmptyCoutingLine()); // reinicia la línea
    };
    
    return (
        <View>
            <View style={{margin : 5}}><Button title="CREAR LINEA" onPress={()=> setAddline(!addLine)} /></View>
            ...
            {addLine && (
                <View   style={[styles.rowContainer, { padding: 3, margin : 5, width : 'auto' }]}>
                    ...
                    <View style={[{ width : '10%'}]}><Button title="Agregar" onPress={ () => SaveLineDetail()} /></View>
                </View>
            )}
            ...
            <FlatList
                ...
                renderItem={({ item }) => (
                    <View style={[styles.rowContainer, { alignContent: 'space-between', margin: 5 }]}>
                        ...
                        <Ionicons name="trash" size={20} color="red" style={{ margin: 10 }} onPress={async () =>await DeleteCoutingLine(item.id)} />
                    </View>
                )}
            />
        </View>
