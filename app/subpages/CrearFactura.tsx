...
export const CrearFacturaSubPage = () => {
  ...
  const _onSaveUserBill = (NewValue_: userBillName) => {
    setDetailUser(NewValue_);
  };
  const _onSaveCompany = (NewValue: Company) => {
    setDetailComp(NewValue);
  };
  const CancelFActura = () => {
    setSavedHead(false);
  };

  const AddCountingLineDet = (value: CoutingDatail) => {
    setCoutingDetail((prev) => [...prev, value]);
  };
  const DeleteCoutingLineDet = (id: number) => {
    setCoutingDetail((prev) => prev.filter((item) => item.id !== id));
  };
  ...
  const GenerateFactura = async () => {
    if (
      ...
      !DetailUser ||
      !DetailComp ||
      CoutingDetail.length === 0
    ) {
      Alert.alert(
        "Error",
        "Faltan datos para generar la factura. Falta Empresa, Cliente"
      );
      return;
    }
    ...
    for (const item of CoutingDetail) {
        console.log('item ', item);
        await addCoutingLineDetail(item);
      }
  ...
  return (
    <View style={{ flex: 1, alignItems: "center" }}>
      <Text style={[styles.bigtitle, { margin: 7 }]}>
        {savedHead ? "FACTURA" : "DETALLE FACTURA"}
      </Text>
      {!savedHead ? (
        <View
          style={[
            styles.card,
            { borderWidth: 1, borderColor: "grey", backgroundColor: "white" },
          ]}
        >
          <HeadFacturaConfig
